// ABOUTME: SSE transport for MCP communication.
// ABOUTME: Connects to HTTP endpoints using Server-Sent Events for bidirectional messaging.
...
mod tests {
    ...
    async fn test_connect_invalid_url() {
        ...
        // Should fail to connect (timeout or connection refused)
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_connect_malformed_url() {
        let result = SseTransport::connect("not-a-valid-url").await;

        // Should fail - can't parse as URL
        assert!(result.is_err());
    }
}
